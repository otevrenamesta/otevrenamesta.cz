...
// Computed
const event = computed(() => events.value.find(({ id }) => id === 9));

// Lifecycle
onMounted(async() => {
  events.value = await useApi.get(`/items/events/?&filter={"id":{"_eq":"9"}}&deep[translations][_filter][languages_code][_eq]=${useI18n()?.locale?.value}&fields=*,translations.*&limit=1`)
    .then((res) => res.data)
    .catch((error) => {
      console.error(error);
      return [];
